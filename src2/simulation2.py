...
def get_circuit_metrics(transpiled_circuit):
    ...
    return {
        'depth': transpiled_circuit.depth(), 'total_gates': sum(gc.values()),
        'gate_counts': gc, 'two_qubit_gates': two_q, 'swap_count': gc.get('swap', 0),
    }
...
def run_dual_simulation(encoding_result, shots=8192):
    ...
    return {
        'aer': {'counts': aer_counts, 'dm': aer_dm, 'metrics': metrics, 'shots': shots},
        'sherbrooke': {'counts': sherbrooke_counts, 'dm': sherbrooke_dm, 'metrics': metrics, 'shots': shots},
    }
